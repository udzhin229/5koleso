...
$stmt_ac = $pdo->prepare("SELECT * FROM models WHERE `type` = 'ac' AND `name` IN ('Чохол на докатку', 'Домкрат + ключ + чохол')");
$stmt_ac->execute();
            ...
            <div class="container">
                ...
                <div class="access__inner" id="access">
                    ...
                    foreach ($result_ac as $product) {
                        ...
                        $descr1_ru = $product['descr1_ru'];
                            if (!empty($descr1_ru)) {
                            $descr_1_ru = explode(";", $descr1_ru);
                        }
                        $descr2_ru = $product['descr2_ru'];
                            if (!empty($descr2_ru)) {
                            $descr_2_ru = explode(";", $descr2_ru);
                        }
                        $descr3_ru = $product['descr3_ru'];
                            if (!empty($descr3_ru)) {
                            $descr_3_ru = explode(";", $descr3_ru);
                        }
                        $l_c = $product['load_capacity'];
                            if (!empty($l_c)) {
                                $load_capacity = explode(";", $l_c);
                            }
                        $r_s = $product['rim_size'];
                            if (!empty($r_s)) {
                        ...
                        $wd = $product['width'];
                            ...
                            echo '
                                    ...
                                    <div class="img">
                                    <img id="main-img_'.$kkk.'" src="data:image/jpeg;base64,' . base64_encode($product['img1']) . '" alt="' . $product['name'] . '">
                                    </div>
                                    <div class="img_blocks">';
                                    if(!empty($product['img1'])){echo '<div class="img_block img_block_'.$kkk.'"><img onclick="imgC('.$kkk.')" src="data:image/jpeg;base64,'.base64_encode( $product['img1'] ).'" alt=""></div>';}
                                    if(!empty($product['img2'])){echo '<div class="img_block img_block_'.$kkk.'"><img onclick="imgC('.$kkk.')" src="data:image/jpeg;base64,'.base64_encode( $product['img2'] ).'" alt=""></div>';}
                                    if(!empty($product['img3'])){echo '<div class="img_block img_block_'.$kkk.'"><img onclick="imgC('.$kkk.')" src="data:image/jpeg;base64,'.base64_encode( $product['img3'] ).'" alt=""></div>';}
                                    echo '</div>
                                    </div>';
                                    echo '<div class="info"';
                                    if (!empty($product['descr1']) && !empty($product['descr3'])) {
                                    echo 'style="flex-direction: unset; flex-wrap: wrap; align-self: flex-start; gap: 0px 30px;"';
                                    }
                                    echo '>';
                                    if(!empty($product['all_descr']) || !empty($product['all_descr_ru'])) {if($lng == 'ru' && $product['all_descr_ru']){echo '<div class="all_descr">'.$product['all_descr_ru'].'</div>';} else {echo '<div class="all_descr">'.$product['all_descr'].'</div>';};}
                                    if (!empty($descr1)) {
                                    echo '<div class="texts" ';
                                    if (!empty($product['descr1']) && empty($product['descr3'])) {
                                    echo 'style="flex-direction: unset; flex-wrap: wrap; max-width: 430px;"';
                                    } else {
                                    echo 'style="max-height: 160px;"';
                                    }
                                    echo '>
                                    <div class="ttl_texts"';
                                    if (!empty($product['ttl_descr1'])) {
                                    echo 'style="display: block;"';
                                    } else {
                                    echo 'style="display: none;"';
                                    }
                                    echo '>'; 
                                    if(!empty($product['ttl_descr1']) || !empty($product['ttl_descr1_ru'])) {if($lng == 'ru' && $product['ttl_descr1_ru']){echo $product['ttl_descr1_ru'];} else {echo $product['ttl_descr1'];};}
                                    echo '</div>';
                                    if ($lng == 'ru') {
                                        foreach ($descr_1_ru as $desc) {
                                            if (!empty($desc)) {
                                                echo "<div class='text'><img src='/assets/images/wheel.svg'>$desc</div>";
                                            }
                                        }
                                    }
                                    else {
                                        foreach ($descr_1 as $desc) {
                                            if (!empty($desc)) {
                                                echo "<div class='text'><img src='/assets/images/wheel.svg'>$desc</div>";
                                            }
                                        }
                                    ...
                                    echo '>
                                    <div class="ttl_texts"';
                                    if (!empty($product['ttl_descr2'])) {
                                    echo 'style="display: block;"';
                                    } else {
                                    echo 'style="display: none;"';
                                    }
                                    echo '>'; 
                                    if(!empty($product['ttl_descr2']) || !empty($product['ttl_descr2_ru'])) {if($lng == 'ru' && $product['ttl_descr2_ru']){echo $product['ttl_descr2_ru'];} else {echo $product['ttl_descr2'];};}
                                    echo'</div>';
                                    if ($lng == 'ru') {
                                        foreach ($descr_2_ru as $desc) {
                                            if (!empty($desc)) {
                                                echo "<div class='text'><img src='/assets/images/wheel.svg'>$desc</div>";
                                            }
                                        }
                                    ...
                                    echo '>
                                    <div class="ttl_texts"';
                                    if (!empty($product['ttl_descr3'])) {
                                    echo 'style="display: block;"';
                                    } else {
                                    echo 'style="display: none;"';
                                    }
                                    echo '>';
                                    if(!empty($product['ttl_descr3']) || !empty($product['ttl_descr3_ru'])) {if($lng == 'ru' && $product['ttl_descr3_ru']){echo $product['ttl_descr3_ru'];} else {echo $product['ttl_descr3'];};} 
                                    echo '</div>';
                                    if ($lng == 'ru') {
                                        foreach ($descr_3_ru as $desc) {
                                            if (!empty($desc)) {
                                                echo "<div class='text'><img src='/assets/images/wheel.svg'>$desc</div>";
                                            }
                                        }
                                    ...
                                    echo '</div>';
                                        ...
                                        echo '<div class="in1">
                                                <div class="deliv">'.$translation['c16'].'</div>
                                                <div class="price">'.$translation['c17'].' <span><p id="price'.$kkk.'">'.$product['price'].'</p> грн</span></div>
                                                <div class="price price_mob"><span>'.$product['price'].' грн</span></div>';
                                                if(!empty($product['width'])) 
                                                {echo '
                                                    <div class="load_c">
                                                        <div class="load_c_min">
                                                        <div class="deliv">'.$translation['c32'].'</div>
                                                        <select name="l_c" id="wd_'.$product['id'].'">
                                                            ';
                                                ...
                                                {echo '
                                                    <div class="load_c">
                                                        ...
                                                        <select name="l_c" id="pr_'.$product['id'].'">
                                                            ';
                                                ...
                                                ';}
                                                if(!empty($product['rim_size'])) 
                                                {echo '
                                                    <div class="load_c">
                                                        <div class="load_c_min">
                                                        <div class="deliv">'.$translation['c30'].'</div>
                                                        <select name="l_c" id="r_s_'.$product['id'].'">
                                                            ';
                                                ...
                                                ';}
                                                if(!empty($product['load_capacity'])) 
                                                {echo '
                                                    <div class="load_c">
                                                        <div class="load_c_min">
                                                        <div class="deliv">'.$translation['c25'].'</div>
                                                        <select name="l_c" id="l_c_'.$product['id'].'" onchange="updatePrice(this, '.$kkk.')">
                                                            ';
                                            ...
                                            <div class="to-cart">
                                                ...
                                                <input type="button" value="'.$translation['c10'].'" onclick="addCart('.$product['id'].', document.getElementById(`price'.$kkk.'`).textContent)">
                                                <input type="button" class="check" onclick=\'location.href="/catalog/good.php?id='.$product['id'].'"\' value="'.$translation['c27'].'">
                                            </div>
                                        </div>
                                    </div>
                                </div>
